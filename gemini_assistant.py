...
TÜRKÇE ve RESMİ cevap ver:"""
    ...
    def auto_sql_agent(self, question):
        ...
        try:
            ...
            - KURAL 10 (Yakıt Verimliliği): En verimli, en az yakan veya en yüksek verimli araçlar sorulursa, bu araçlar HER ZAMAN şirket bünyesindeki özmal araçlar olmalıdır. Dolayısıyla, `yakit` tablosunu `araclar` tablosuyla `plaka` sütunu üzerinden INNER JOIN yapıp `araclar.sahip = 'BİZİM'` filtresi uygulayarak (taşeron araçlarını tamamen hariç tutarak) `litre_km` sütununun ortalaması en düşük (ve 0'dan büyük) olan araçları getir.
